...
def export_zip(brief: dict, results: dict, analytics: dict) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        # --- PRO ADDITION: include voiceover mp3 in ZIP ---
        if results.get("voiceover_path") and os.path.isfile(str(results["voiceover_path"])):
            z.write(results["voiceover_path"], "voiceover.mp3")
        if results.get("voiceover_script"):
            z.writestr("voiceover_script.txt", results["voiceover_script"])
        # --- PRO ADDITION: include critic verdicts in ZIP ---
        if results.get("critic_verdicts"):
            z.writestr("critic_verdicts.json", json.dumps(results["critic_verdicts"], indent=2))
        if analytics:
